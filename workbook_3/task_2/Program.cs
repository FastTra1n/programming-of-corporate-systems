...
while (true)
{
    ...
    try
    {
        ...
        if (m > 3 || n > 10)
        {
            Console.WriteLine("Введённые значения для переменных m и n слишком велики и могут привести к непредсказуемому результату (неточность ответа/ошибка). Попробуйте другие, меньшие значения (m <= 3 и n <= 10).");
            continue;
        }
        result = A(m, n);
